...
/**
 * Complete edit user tag template.
 */
...
<div class="container p-0">
    <nav class="py-3">
        <span class="navbar-brand mb-0 h1">Edit user tag</span>
    </nav>

    <?php if ($this->session->flashdata('editUserTagSuccess') === true): ?>
    <div class="alert alert-success alert-dismissible fade show animated faster fadeIn" role="alert" _auto_close="15000">
        <i class="fas fa-check-circle mr-1"></i> The changes were successfully saved. <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3)]); ?>">check all user tags</a>.
    ...
    <?php if ($this->session->flashdata('editUserTagSuccess') === false): ?>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userTagName')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userTagName" name="userTagName" value="<?php echo ($this->input->post('userTagName') === null ? $userTag->tagName : set_value('userTagName')); ?>" aria-describedby="userTagNameHelp" placeholder="Enter user tag name" autofocus required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userTagSlug')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userTagSlug" name="userTagSlug" value="<?php echo ($this->input->post('userTagSlug') === null ? $userTag->tagSlug : set_value('userTagSlug')); ?>" aria-describedby="userTagSlugHelp" placeholder="Enter user tag slug" required>
                ...
                <div class="form-group mb-0">
                    ...
                    <textarea class="form-control" id="userTagDescription" name="userTagDescription" rows="4" placeholder="Enter user tag description"><?php echo ($this->input->post('userTagDescription') === null ? $userTag->tagDescription : set_value('userTagDescription')); ?></textarea>
                </div>
            </div>
        </div>
            ...
            <div id="state" class="collapse">
                <div class="card-body">
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="stateActive" name="state" value="active" class="custom-control-input <?php echo (isset($_POST['submit']) ? (empty(form_error('state')) ? 'is-valid' : 'is-invalid') : ''); ?>" <?php echo ($this->input->post('state') === null ? ($userTag->state == 'active' ? 'checked' : '') : set_radio('state', 'active')); ?> required>
                        <label class="custom-control-label" for="stateActive">Active</label>
                    </div>

                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="stateInactive" name="state" value="inactive" class="custom-control-input <?php echo (isset($_POST['submit']) ? (empty(form_error('state')) ? 'is-valid' : 'is-invalid') : ''); ?>" <?php echo ($this->input->post('state') === null ? ($userTag->state == 'inactive' ? 'checked' : '') : set_radio('state', 'inactive')); ?>>
