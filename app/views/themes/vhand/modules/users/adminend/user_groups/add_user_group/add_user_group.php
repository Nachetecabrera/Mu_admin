...
/**
 * Complete add new user group template.
 */
...
<div class="container p-0">
    <nav class="py-3">
        <span class="navbar-brand mb-0 h1">Add user group</span>
    </nav>

    <?php if ($this->session->flashdata('addUserGroupSuccess') === true): ?>
    <div class="alert alert-success alert-dismissible fade show animated faster fadeIn" role="alert" _auto_close="15000">
        <i class="fas fa-check-circle mr-1"></i> User group was successfully added. <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3), 'edit', $this->session->flashdata('insertID')]); ?>">edit that user group</a> or <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3)]); ?>">check all user groups</a>.
    ...
    <?php if ($this->session->flashdata('addUserGroupSuccess') === false): ?>
    <div class="alert alert-danger alert-dismissible fade show animated faster shake" role="alert" _auto_close="15000">
        <i class="fas fa-times-circle mr-1"></i> Add user group process was unsuccessful due to an internal error. please try again later.
        <button type="button" class="close" data-dismiss="alert" aria-label="Close">
            <i class="fas fa-times"></i>
        </button>
    </div>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userGroupName')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userGroupName" name="userGroupName" value="<?php echo set_value('userGroupName'); ?>" aria-describedby="userGroupNameHelp" placeholder="Enter user group name" autofocus required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userGroupSlug')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userGroupSlug" name="userGroupSlug" value="<?php echo set_value('userGroupSlug'); ?>" aria-describedby="userGroupSlugHelp" placeholder="Enter user group slug" required>
                ...
                <div class="form-group mb-0">
                    ...
                    <textarea class="form-control" id="userGroupDescription" name="userGroupDescription" rows="4" placeholder="Enter user group description"><?php echo set_value('userGroupDescription'); ?></textarea>
                </div>
            </div>
        </div>
