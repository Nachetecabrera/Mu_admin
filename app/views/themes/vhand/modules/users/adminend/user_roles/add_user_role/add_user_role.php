...
/**
 * Complete add new user role template.
 */
...
<div class="container p-0">
    <nav class="py-3">
        <span class="navbar-brand mb-0 h1">Add user role</span>
    </nav>

    <?php if ($this->session->flashdata('addUserRoleSuccess') === true): ?>
    <div class="alert alert-success alert-dismissible fade show animated faster fadeIn" role="alert" _auto_close="15000">
        <i class="fas fa-check-circle mr-1"></i> User role was successfully added. <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3), 'edit', $this->session->flashdata('insertID')]); ?>">edit that user role</a> or <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3)]); ?>">check all user roles</a>.
    ...
    <?php if ($this->session->flashdata('addUserRoleSuccess') === false): ?>
    <div class="alert alert-danger alert-dismissible fade show animated faster shake" role="alert" _auto_close="15000">
        <i class="fas fa-times-circle mr-1"></i> Add user role process was unsuccessful due to an internal error. please try again later.
        <button type="button" class="close" data-dismiss="alert" aria-label="Close">
            <i class="fas fa-times"></i>
        </button>
    </div>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userRoleName')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userRoleName" name="userRoleName" value="<?php echo set_value('userRoleName'); ?>" aria-describedby="userRoleNameHelp" placeholder="Enter user role name" autofocus required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userRoleSlug')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userRoleSlug" name="userRoleSlug" value="<?php echo set_value('userRoleSlug'); ?>" aria-describedby="userRoleSlugHelp" placeholder="Enter user role slug" required>
                ...
                <div class="form-group mb-0">
                    ...
                    <textarea class="form-control" id="userRoleDescription" name="userRoleDescription" rows="4" placeholder="Enter user role description"><?php echo set_value('userRoleDescription'); ?></textarea>
                </div>
            </div>
        </div>
