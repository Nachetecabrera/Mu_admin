...
/**
 * Complete edit user status template.
 */
...
<div class="container p-0">
    <nav class="py-3">
        <span class="navbar-brand mb-0 h1">Edit user status</span>
    </nav>

    <?php if ($this->session->flashdata('editUserStatusSuccess') === true): ?>
    <div class="alert alert-success alert-dismissible fade show animated faster fadeIn" role="alert" _auto_close="15000">
        <i class="fas fa-check-circle mr-1"></i> The changes were successfully saved. <a class="alert-link" href="<?php echo site_url([$this->uri->segment(1), $this->uri->segment(2), $this->uri->segment(3)]); ?>">check all user statuses</a>.
    ...
    <?php if ($this->session->flashdata('editUserStatusSuccess') === false): ?>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userStatusName')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userStatusName" name="userStatusName" value="<?php echo ($this->input->post('userStatusName') === null ? $userStatus->statusName : set_value('userStatusName')); ?>" aria-describedby="userStatusNameHelp" placeholder="Enter user status name" autofocus required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?php echo (isset($_POST['submit']) ? (empty(form_error('userStatusSlug')) ? 'is-valid' : 'is-invalid') : ''); ?>" id="userStatusSlug" name="userStatusSlug" value="<?php echo ($this->input->post('userStatusSlug') === null ? $userStatus->statusSlug : set_value('userStatusSlug')); ?>" aria-describedby="userStatusSlugHelp" placeholder="Enter user status slug" required>
                ...
                <div class="form-group mb-0">
                    ...
                    <textarea class="form-control" id="userStatusDescription" name="userStatusDescription" rows="4" placeholder="Enter user status description"><?php echo ($this->input->post('userStatusDescription') === null ? $userStatus->statusDescription : set_value('userStatusDescription')); ?></textarea>
                </div>
            </div>
        </div>
            ...
            <div id="state" class="collapse">
                <div class="card-body">
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="stateActive" name="state" value="active" class="custom-control-input <?php echo (isset($_POST['submit']) ? (empty(form_error('state')) ? 'is-valid' : 'is-invalid') : ''); ?>" <?php echo ($this->input->post('state') === null ? ($userStatus->state == 'active' ? 'checked' : '') : set_radio('state', 'active')); ?> required>
                        <label class="custom-control-label" for="stateActive">Active</label>
                    </div>

                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="stateInactive" name="state" value="inactive" class="custom-control-input <?php echo (isset($_POST['submit']) ? (empty(form_error('state')) ? 'is-valid' : 'is-invalid') : ''); ?>" <?php echo ($this->input->post('state') === null ? ($userStatus->state == 'inactive' ? 'checked' : '') : set_radio('state', 'inactive')); ?>>
