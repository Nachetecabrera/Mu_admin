...
class User_roles_model extends CI_Model
{
     ...
     */
    public function userRoles($selectFields, $defaultOrderBy, $defaultOrder, $perPage = false, $searchFields = [], $filterFields = [])
    {
        /**
         * Compiled sub queries.
         */

        // For aliased userRolesWithRelationsTable.
        $this->db->select('users_user_roles.ID AS userRoleID, COUNT(roleID) AS userCount');
        $this->db->from('users_user_roles');
        $this->db->join('users_user_role_relations', 'users_user_role_relations.roleID = users_user_roles.ID', 'left');
        $this->db->group_by('users_user_roles.ID');

        $subQueryUserRolesWithRelationsTable = $this->db->get_compiled_select();
        ...
        $this->db->join('(' . $subQueryUserRolesWithRelationsTable . ') AS userRolesWithRelationsTable', 'userRolesWithRelationsTable.userRoleID = users_user_roles.ID', 'left');
