...
class User_statuses_model extends CI_Model
{
    /**
     ...
     */
    public function userStatuses($selectFields, $defaultOrderBy, $defaultOrder, $perPage = false, $searchFields = [], $filterFields = [])
    {
        /**
         * Compiled sub queries.
         */

        // For aliased usersTable.
        $this->db->select('users_user_statuses.ID AS userStatusID, COUNT(statusID) AS userCount');
        $this->db->from('users_user_statuses');
        $this->db->join('users_users', 'users_users.statusID = users_user_statuses.ID', 'left');
        $this->db->group_by('users_user_statuses.ID');

        $subQueryUsersTable = $this->db->get_compiled_select();
        ...
        $this->db->join('(' . $subQueryUsersTable . ') AS userStatusesWithUsersTable', 'userStatusesWithUsersTable.userStatusID = users_user_statuses.ID', 'left');
        ...
        $queryResult = $this->db->get('users_user_statuses');
