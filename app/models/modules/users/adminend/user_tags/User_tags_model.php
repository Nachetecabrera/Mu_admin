...
class User_tags_model extends CI_Model
{
    /**
     * Get all user tags.
     *
     ...
     */
    public function userTags($selectFields, $defaultOrderBy, $defaultOrder, $perPage = false, $searchFields = [], $filterFields = [])
    {
        /**
         * Compiled sub queries.
         */

        // For aliased userTagsWithRelationsTable.
        $this->db->select('users_user_tags.ID AS userTagID, COUNT(tagID) AS userCount');
        $this->db->from('users_user_tags');
        $this->db->join('users_user_tag_relations', 'users_user_tag_relations.tagID = users_user_tags.ID', 'left');
        $this->db->group_by('users_user_tags.ID');

        $subQueryUserTagsWithRelationsTable = $this->db->get_compiled_select();
        ...
        $this->db->join('(' . $subQueryUserTagsWithRelationsTable . ') AS userTagsWithRelationsTable', 'userTagsWithRelationsTable.userTagID = users_user_tags.ID', 'left');
        ...
        $queryResult = $this->db->get('users_user_tags');
